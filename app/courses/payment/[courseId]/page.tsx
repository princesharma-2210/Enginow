...
// Replace with actual course data fetching logic later
const courseData: Record<string, { title: string; duration: string; price: number; originalPrice: number }> = {
...
export default function PaymentPage() {
  ...
  const program = courseData[courseId]

  if (!program) {
    return (
      <div className="container py-20 text-center">
        <h1 className="text-2xl font-semibold">Course Not Found</h1>
        <p className="text-muted-foreground mt-2">The course you're trying to access does not exist.</p>
        <Link href="/training">
          <Button className="mt-6">Back to Training Programs</Button>
        </Link>
      </div>
    )
  }
  ...
  const handlePayment = async () => {
    // Implement Razorpay or Stripe integration here
    alert(`Proceed to pay ₹${program.price} for ${program.title} (Enrollment ID: ${enrollmentId})`)
  }
